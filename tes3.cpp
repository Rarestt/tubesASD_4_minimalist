...
void input_all(Node *root), filterKat_Subkat(Node *root), filterHarga(Node *root), filterEvent(Node *root), traverseTree(Node *root, int level),
     ...
     findSubcategory(Node* root, const string& subcategory, Node*& subcategoryNode), filterBySubcategory(Node* root), filterByMaxPrice(Node *root, int budget),
     traverseEvent(Node *root, int level), modifyCategory(Node* root, const string& oldCategory, const string& newCategory),
...
//Filter Subkategori
void findSubcategory(Node* root, string& subcategory, Node*& subcategoryNode){
    if(root == nullptr){
        return;
    }
